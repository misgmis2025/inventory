...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_item']) && isset($_SESSION['usertype']) && $_SESSION['usertype'] === 'admin') {
        ...
        $quantity = max(0, intval($_POST['quantity'] ?? 1));
        $location = trim($_POST['location'] ?? '');
        // condition removed from edit flow; keep existing
        $status = trim($_POST['status'] ?? 'Available');
        $date_acquired = trim($_POST['date_acquired'] ?? '');
        $remarks = trim($_POST['remarks'] ?? '');
        $serial_no = trim($_POST['serial_no'] ?? '');
        if ($item_name === '' && $model !== '') { $item_name = $model; }
        // MySQL uniqueness check for serial_no
        if ($serial_no !== '') {
            if ($chk = $conn->prepare("SELECT COUNT(*) FROM inventory_items WHERE serial_no = ? AND id <> ?")) {
                $chk->bind_param('si', $serial_no, $id_edit);
                $chk->execute();
                $chk->bind_result($cnt);
                $cnt = 0; $chk->fetch(); $chk->close();
                if (intval($cnt,10) > 0) { http_response_code(400); echo 'Serial ID already exists for another item.'; exit(); }
            }
        }
        if ($id_edit > 0 && $item_name !== '') {
            $stmt = $conn->prepare("UPDATE inventory_items
                SET item_name=?, category=?, model=?, quantity=?, location=?, serial_no=?, status=?,
                    date_acquired = CASE WHEN ? = '' THEN date_acquired ELSE ? END,
                    remarks=?
                WHERE id=?");
            $stmt->bind_param('sssissssssi', $item_name, $category, $model, $quantity, $location, $serial_no, $status, $date_acquired, $date_acquired, $remarks, $id_edit);
            if ($stmt->execute()) {
              $item_updated = true;
              // If item was marked as Lost, adjust Mongo borrowable lists
              if (strcasecmp($status, 'Lost') === 0) {
                  try {
                      @require_once __DIR__ . '/../vendor/autoload.php';
                      @require_once __DIR__ . '/db/mongo.php';
                      $mdb = get_mongo_db();
                      if ($mdb) {
                          $buCol = $mdb->selectCollection('borrowable_units');
                          $bcCol = $mdb->selectCollection('borrowable_catalog');
                          $buCol->deleteOne(['model_id' => (int)$id_edit]);
                          $mname = ($model !== '' ? $model : $item_name);
                          $catNorm = ($category !== '' ? $category : 'Uncategorized');
                          if ($mname !== '') {
                              $cur = $bcCol->findOne(['model_name'=>$mname,'category'=>$catNorm], ['projection'=>['borrow_limit'=>1]]);
                              if ($cur && isset($cur['borrow_limit'])) {
                                  $new = max(0, (int)$cur['borrow_limit'] - 1);
                                  $bcCol->updateOne(['model_name'=>$mname,'category'=>$catNorm], ['$set'=>['borrow_limit'=>$new]]);
                              }
                          }
                      }
                  } catch (Throwable $e) { /* ignore mongo adjust */ }
              }
          }
          $stmt->close();
        }
    ...
    if (isset($_POST['bulk_delete']) && isset($_SESSION['usertype']) && $_SESSION['usertype'] === 'admin') {
        ...
        if (is_array($ids) && !empty($ids)) {
            ...
            foreach ($ids as $rawId) {
                $itemId = intval($rawId);
                if ($itemId <= 0) { continue; }
                // Log snapshot before delete (skip 'In Use')
                if ($ins = $conn->prepare("INSERT INTO inventory_delete_log
                    (item_id, deleted_by, reason, item_name, model, category, quantity, status, serial_no)
                    SELECT id, ?, ?, item_name, model, COALESCE(NULLIF(category,''),'Uncategorized'), quantity, status, serial_no
                    FROM inventory_items WHERE id=? AND status <> 'In Use'")) {
                    $ins->bind_param('ssi', $by, $reason, $itemId);
                    $ins->execute();
                    $ins->close();
                }
                // Delete item
                if ($del = $conn->prepare("DELETE FROM inventory_items WHERE id = ? AND status <> 'In Use'")) {
                    $del->bind_param("i", $itemId);
                    $del->execute();
                    if ($del->affected_rows > 0) { $deletedCount++; }
                    $del->close();
                }
            }
        }
    ...
    if (isset($_SESSION['usertype']) && $_SESSION['usertype'] === 'admin') {
        ...
        if (isset($_POST['bm_add'])) {
            ...
            $bm_model = trim($_POST['bm_model'] ?? '');
            if ($bm_cat !== '' && $bm_model !== '') {
                $stmt = $conn->prepare("INSERT INTO borrowable_models (model_name, category, active) VALUES (?, ?, 1) ON DUPLICATE KEY UPDATE active=VALUES(active)");
                if ($stmt) { $stmt->bind_param('ss', $bm_model, $bm_cat); $stmt->execute(); $stmt->close(); }
            }
        }
        // Add ALL models in a category (bulk activate)
        if (isset($_POST['bm_add_all'])) {
            $bm_cat = trim($_POST['bm_category'] ?? '');
            if ($bm_cat !== '') {
                // fetch distinct models from available inventory for this category
                $q = "SELECT DISTINCT COALESCE(NULLIF(model,''), item_name) AS model_name
                      FROM inventory_items
                      WHERE COALESCE(NULLIF(category,''),'Uncategorized') = ?
                        AND status='Available' AND quantity > 0";
                $ps = $conn->prepare($q);
                if ($ps) {
                    $ps->bind_param('s', $bm_cat);
                    if ($ps->execute()) {
                        $res = $ps->get_result();
                        while ($r = $res->fetch_assoc()) {
                            $mn = trim($r['model_name'] ?? '');
                            if ($mn === '') continue;
                            $ins = $conn->prepare("INSERT INTO borrowable_models (model_name, category, active) VALUES (?, ?, 1) ON DUPLICATE KEY UPDATE active=VALUES(active)");
                            if ($ins) { $ins->bind_param('ss', $mn, $bm_cat); $ins->execute(); $ins->close(); }
                        }
                    }
                    $ps->close();
                }
        ...
        if (isset($_POST['bm_remove'])) {
            ...
            $bm_model = trim($_POST['bm_model'] ?? '');
            if ($bm_cat !== '' && $bm_model !== '') {
                $stmt = $conn->prepare("UPDATE borrowable_models SET active=0 WHERE model_name=? AND category=?");
                if ($stmt) { $stmt->bind_param('ss', $bm_model, $bm_cat); $stmt->execute(); $stmt->close(); }
            }
        }
    }
