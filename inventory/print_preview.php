...
if (!$usedMongo) {
  ...
  // If reservation mode, build from current reservations instead of inventory_items
  if ($reservationMode) {
    $items = [];
    $sql = "SELECT er.item_name, COALESCE(NULLIF(ii.category,''),'Uncategorized') AS category,
                   SUM(er.quantity) AS qty, COALESCE(ii.location, '') AS location
            FROM equipment_requests er
            LEFT JOIN inventory_items ii ON (ii.model = er.item_name OR ii.item_name = er.item_name)
            WHERE er.type='reservation' AND er.status='Approved'
              AND er.reserved_from <= NOW() AND er.reserved_to >= NOW()
            GROUP BY er.item_name, COALESCE(NULLIF(ii.category,''),'Uncategorized'), COALESCE(ii.location, '')
            ORDER BY COALESCE(NULLIF(ii.category,''),'Uncategorized') ASC, er.item_name ASC";
    ...
    $sql = "SELECT id, item_name, category, quantity, location, `condition`, status, date_acquired FROM inventory_items WHERE 1";
  $params = [];
  $types = '';
  if ($search_q !== '') {
  ...
  if ($filter_status !== '') {
    if ($filter_status === 'Out of Order') { $sql .= " AND quantity = 0"; }
    else { $sql .= " AND status = ?"; $params[] = $filter_status; $types .= 's'; }
  }
  ...
  if ($ldShow && !empty($items)) {
    ...
    if (!empty($modelIds)) {
      ...
      // Markers
      $sqlLD = "SELECT l.model_id, l.username AS marker_username, l.created_at FROM lost_damaged_log l WHERE l.model_id IN ($idList) AND l.action = ? ORDER BY l.id DESC";
      if ($stLD = $conn->prepare($sqlLD)) {
        $stLD->bind_param('s', $filter_status);
        if ($stLD->execute()) { $resLD=$stLD->get_result(); $seen=[]; while($r=$resLD->fetch_assoc()){ $mid=(int)$r['model_id']; if(isset($seen[$mid])) continue; $seen[$mid]=true; $ldExtraMap[$mid] = ['marker_username'=>(string)$r['marker_username'], 'marker_full_name'=>'', 'by_username'=>'', 'by_full_name'=>'', 'log_created_at'=>(string)($r['created_at'] ?? '')]; } }
        $stLD->close();
      }
      ...
      if (!empty($ldExtraMap)) {
        ...
        if (!empty($usernames)) {
          ...
          $sqlU = "SELECT username, COALESCE(NULLIF(full_name,''), username) AS full_name FROM users WHERE username IN ($in)";
          if ($stU = $conn->prepare($sqlU)) {
            $stU->bind_param($types, ...$usernames);
            if ($stU->execute()) { $resU=$stU->get_result(); $map=[]; while($ur=$resU->fetch_assoc()){ $map[(string)$ur['username']] = (string)$ur['full_name']; } foreach ($ldExtraMap as $mid=>&$e){ $u=(string)($e['marker_username']??''); $e['marker_full_name'] = $map[$u] ?? $u; } unset($e); }
            $stU->close();
          }
        }
      }
      // Responsible borrower at or before the log time per model; leave blank if none or no log
      foreach ($ldExtraMap as $mid => $e) {
        $midI = (int)$mid; if ($midI <= 0) continue; $logTs = trim((string)($e['log_created_at'] ?? '')); if ($logTs === '') continue;
        $qUB = "SELECT ub.username FROM user_borrows ub WHERE ub.model_id = ? AND ub.returned_at IS NOT NULL AND ub.returned_at <= ? ORDER BY ub.returned_at DESC, ub.id DESC LIMIT 1";
        if ($stB = $conn->prepare($qUB)) {
          $stB->bind_param('is', $midI, $logTs);
          if ($stB->execute()) { $rs=$stB->get_result(); if ($rowB=$rs->fetch_assoc()){ $ldExtraMap[$mid]['by_username'] = (string)$rowB['username']; } }
          $stB->close();
        }
      }
      ...
      if (!empty($userSet)) {
        $ua = array_keys($userSet); $in = implode(",", array_fill(0,count($ua),'?')); $types=str_repeat('s', count($ua));
        $sqlN = "SELECT username, COALESCE(NULLIF(full_name,''), username) AS full_name FROM users WHERE username IN ($in)";
        if ($stN = $conn->prepare($sqlN)) { $stN->bind_param($types, ...$ua); if ($stN->execute()){ $resN=$stN->get_result(); $map=[]; while($ur=$resN->fetch_assoc()){ $map[(string)$ur['username']] = (string)$ur['full_name']; } foreach($ldExtraMap as $mid=>&$e){ $u=(string)($e['by_username']??''); if ($u!=='') { $e['by_full_name'] = $map[$u] ?? $u; } } unset($e);} $stN->close(); }
      }
    }
  }
}
